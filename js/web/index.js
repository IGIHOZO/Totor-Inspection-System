...
callAuto();
function celClasses(){
	//document.getElementById("att_class").innerHTML="<option>1</option><option>2</option><option>3</option>";
	var celClasses = true;
	// $.ajax({url:"../js/ajax/index.php",
	// 	type:"GET",data:{celClasses:celClasses},cache:false,success:function(res){$("#att_class").html(res);}
	// 	});
		$.ajax({url:"../js/ajax/index.php",
		type:"GET",data:{celClasses:celClasses},cache:false,success:function(res){$("#att_class").html(res);}
		});
...
function myPush(title,body,icon,){
    ...
    Push.create('Hello '+title+'!         Tutor Inspection System',{
      body:body,
      icon:icon,
      timeout:40000,
      onclick:function(){
...
//======================================================================================== USER COMMENT
function subComment(){
	var msg = gtIdVal("com_txt");
	if (isEmpty(msg)) {
		setCont('respp','Please enter your comment ...');
	}else{
		var subComment = true;
		$.ajax({url:"../js/ajax/index.php",
		type:"GET",data:{subComment:subComment,msg:msg},cache:false,success:function(res){$("#respp").html(res);}
		});
	}
}
...
function removeOrientTchr(){
	...
	if (ortTchr == "choose" || ortTchr=="" || ortCls == "choose" || ortCls == "" || ortCrs == "choose" || ortCrs == "") {
		...
		if (ortCrs == "No") {
			...
			var removeOrTchhFn=1;
			$("#okset").attr("disabled","disabled");
			$.ajax({url:"../js/ajax/index.php",
			type:"GET",data:{removeOrTchhFn:removeOrTchhFn,ortTchr:ortTchr,ortCls:ortCls,ortCrs:ortCrs},cache:false,success:function(res){$("#hdn_crt_nw_cls").html(res);}
			});
		}
	}
}
...
$("#remove_or_tch_cls option,#remove_or_tch_cls").click(function(){
	var orThCls=document.getElementById("remove_or_tch_cls").value;
	var orThTchr=document.getElementById("remove_or_tch_tchr").value;
	var removeOrThrCrs=1;
		$.ajax({url:"../js/ajax/index.php",
		type:"GET",data:{removeOrThrCrs:removeOrThrCrs,orThCls:orThCls,orThTchr:orThTchr},cache:false,success:function(res){$("#or_tch_crs").html(res);}
		});
})
