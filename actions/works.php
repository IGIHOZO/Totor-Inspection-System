...
<body class="fixed-nav sticky-footer bg-dark" id="page-top" style="background-color: #000" style="position: absolute;">
  ...
  <div class="content-wrapper">
    <div class="container-fluid">
        ...
        <div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="modalRegisterWork" class="modal fade" style="margin-top: -70px;">
            <div class="modal-dialog">
                <div class="modal-content" style="padding: 20px;">
                    <div class="modal-header">
                         <h4 class="modal-title">New Work registration form</h4>
                  <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                         </div>
                         <span id="hdn_crt_nw_cls"></span>
                            <div class="modal-body">
      <div id="regWorkResponse"></div>
          <form name="worksregoform" action="" method="post" novalidate>
          ...
          <label>Select Class</label>
              ...
              $se_crs=mysql_query("SELECT DISTINCT tis_courses.course_class,tis_classes.class_id,tis_classes.class_name FROM tis_courses,tis_classes WHERE tis_courses.course_class = tis_classes.class_id AND tis_courses.course_teacher='$tchr_id' AND tis_classes.class_xkul='$usr_id' AND LEFT(tis_courses.course_date,4)='".date("Y")."' ORDER BY tis_classes.class_name")or die(mysql_error());
              if (mysql_num_rows($se_crs)>0) {
                while ($ft_se_crs=mysql_fetch_assoc($se_crs)) {

                  echo "<option value='".$ft_se_crs['class_id']."'>".$ft_se_crs['class_name']."</option>";
                }
              }else{
                echo "<option value='choose'>Error AE n-4 Found</option>";
              }
              echo "</select";
              ?>
              </div>
      <input type="hidden" id="tskhddn" value="workss">
      <span></span>
      <div class="form-group">
...
&nbsp;&nbsp;&nbsp;<br><br>
      ...
      <div class="form-group">
        <label>Work Title</label><label style="margin-left: 10%;color: #f45;font-size: 15px;">&nbsp;(&nbsp; Maximum characters: <span style="color: #435344;font-weight: bolder"> 25 characters</span>)<label id="cnt_chr_resp"></label></label>
        <input type="text" class="form-control" oninput="return countTskTtlChrs()" id="tskttl" placeholder="Given Work" maxlength="25">
      </div>
      <div class="form-group">
        ...
        <input type="number" class="form-control" id="tskovll" placeholder="" style="font-size: ">
                        ...
                        <div class="modal-footer" style="margin-top: -30px;">
                            <button class="btn btn-primary" onclick="return setTsk()" id="sttsk"><span class="glyphicon glyphicon-ok"></span>&nbsp;&nbsp; Set Work</button>
		...
		<div aria-hidden='true' aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="modalUpdateWork" class="modal fade">
            <div class='modal-dialog'>
                <div class="modal-content" style="padding: 20px;">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                        <h4 class="modal-title">Update Work form</h4>
                    </div>
                     <div class="modal-body">
      <div id="updWorkResponse"></div>
          <form name="worksupdform" action="" method="post" novalidate>
                    <div class="form-group">
          <label>Select Class</label>
          <?php
              $tchr_id=$_SESSION['seveeen_tis_teacher_id'];
              ...
              if ($cnt_se_crs>0) {
                ...
                while ($ft_se_crs=mysql_fetch_assoc($se_crs)) {
                  ...
                  $sel_clss=mysql_query("SELECT * FROM tis_classes WHERE class_xkul='$usr_id' AND LEFT(class_date,4)='".date("Y")."' AND class_id='$cls_idd'");
              ...
              echo "</select";
              ?></div>
      <input type="hidden" id="updtskhddn" value="workss">
      <div class="form-group">
        <label>Select Lesson</label>
...
&nbsp;&nbsp;&nbsp;<br><br>
      ...
      <div class="form-group">
        <label>Work Title</label><label style="margin-left: 10%;color: #f45;font-size: 15px;">&nbsp;(&nbsp; Maximum characters: <span style="color: #435344;font-weight: bolder"> 25 characters</span>)<label id="updcnt_chr_resp"></label></label>
        <input type="text" class="form-control" oninput="return countTskTtlChrs()" id="tskttl" placeholder="Given Work" maxlength="25">
      </div>
      <div class="form-group">
        <label>Overall marks</label>
        <input type="number" class="form-control" id="updtskovll" placeholder="" style="font-size: ">
                        ...
                        <div class="modal-footer">
                            <button class="btn btn-success" id="btnUpdWork" type="button"><i class="fa fa-check-square-o"></i> Update</button>
                            <button data-dismiss="modal" class="btn btn-danger" type="button"><i class="fa fa-remove"></i>Cancel</button>
                        </div>
                        </div>
                    </div>
		      ...
		      <div class="row" id="workviewform" style="padding:2%;margin-top: -3%">
        <form action="" method="GET">
          <div class="col-lg-12 input-goup">
            <!--input type="text" name="keyWork" id="keyWork" placeholder="Work name..." class="form-control srch"-->
          </div></form>
        <span style="color:green"></span>
        <!-- Example DataTables Card-->

        <div class="table"><button class="btn btn-primary glyphicon glyphicon-plus-sign pull-right" data-toggle='modal' data-target='#modalRegisterWork' style="margin-right:17px;"> New</button>
       <h4>List of Your Works Available</h4>
        <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0" id="tblWorks">
          <thead>
            <tr>
              <th>#Counts</th>
              <th>Class </th>
...
<button data-toggle="modal" data-target="#modalWork" class="btn btn-primary pull-right" style="margin-right:3%;margin-bottom:0.5%;margin-top: -1%"><span class="glyphicon glyphicon-plus"></span>Add Works</button></th-->
            </tr>
          </thead>
          <tbody id="loadedworks">
		  <?php
$ntstt="Ny";
$mdstt="Md";
$sel_tasts=mysql_query("SELECT * FROM tis_tasks WHERE task_teacher='$tchr_id' AND task_xkul='$usr_id' AND task_type='workss' AND ((task_status='$mdstt') OR (task_status='$ntstt')) AND tis_tasks.task_date BETWEEN '".$calendarStart."' AND '".$calendarStop."' ORDER BY task_date DESC");
...
if ($cnt_sel_tasts>0) {
  ...
  while ($ft_sel_tasts=mysql_fetch_assoc($sel_tasts)) {
    ...
    $selcrs=mysql_query("SELECT * FROM tis_courses WHERE course_id='$cors_id' AND course_xkul='$usr_id' AND LEFT(course_date,4)='".date("Y")."'")or die(mysql_error());
      ...
      <div class="modal-content">
        <form >
         ...
         <h4 class="modal-title" id="delModalTitle">Do you want to delete</h4>
          ...
          <div class="modal-footer">
            <button type="button" id="btnDelWork" class="btn btn-danger" ><i class="fa fa-check-square-o"></i>
